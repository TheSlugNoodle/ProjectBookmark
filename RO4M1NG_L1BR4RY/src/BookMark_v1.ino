...
void handleRoot() {
    ...
    */
    ...
    html += "<div style='text-align: center; padding-left: 20px;'>";
    html += "[ System Status: " + String(ledState ? "ACTIVE" : "Rebel") + " ]</p>";
    html += "<h3>HUMAN KNOWLEDGE</h3></p>";
    html += "<h3>BELONGS TO THE WORLD</h3></p>";
    html += "</div>";
...
void handleNodeFiles() {
    ...
    //bool isLocal = (nodeSSID == AP_SSID); /used for handling remote files
    
    // If this is a remote node, ensure we're connected
    if (!isLocal && section == "") {
        if (!connectToPeer(nodeSSID)) {
            // If connection fails, redirect back to node list
    ...
    server.sendContent("<h3>//NODE: " + nodeSSID + "//</h3>");
    ...
    // Wrap content in a div that can be hidden/shown
    server.sendContent("<div id='content' style='display: none;'>");

    if (section == "") {
        server.sendContent("<div class='section-list'>");
        server.sendContent("<a href='/node-files?node=" + nodeSSID + "&section=num' class='section-button' onclick='showLoader()'>[0-9]</a>");
        server.sendContent("<a href='/node-files?node=" + nodeSSID + "&section=sym' class='section-button' onclick='showLoader()'>[#@]</a>");
        for (char c = 'A'; c <= 'Z'; c++) {
            server.sendContent("<a href='/node-files?node=" + nodeSSID + "&section=" + 
                             String(c) + "' class='section-button' onclick='showLoader()'>[" + String(c) + "]</a>");
        }
        server.sendContent("</div>");

        // Centered navigation bar
        server.sendContent("<div class='nav-bar' style='text-align: center;'>");
        server.sendContent("<div style='display: inline-block;'>");
        //server.sendContent("<a href='/list' class='nav-button' onclick='showLoader()'>&lt;&lt; Node List</a>");
        server.sendContent("<a href='/' class='nav-button' onclick='showLoader()'>&lt;&lt; Return to Terminal </a>");
        server.sendContent("</div>");
    } else {
        ...
        // Determine directory path and section title
        if (section == "num") {
            dirPath += "0-9";
            sectionTitle = "[0-9]";
        } else if (section == "sym") {
        ...
        if (isLocal) {
            ...
            for (const auto& remoteFile : remoteFiles) {
                if (String(remoteFile.nodeSSID) == nodeSSID) {
                    String filePath = String(remoteFile.filepath);
                    // Extract the directory and filename
                    int lastSlash = filePath.lastIndexOf('/');
                    String fileName = filePath.substring(lastSlash + 1);
                    String fileDir = filePath.substring(0, lastSlash);
                    
                    // Check if this file belongs in the current section
                    if (fileDir == dirPath && isAllowedFile(fileName)) {
                        sectionFiles.push_back(fileName);
                        Serial.println("Added remote file to section: " + fileName);
                    }
                }
                yield();
            }
        ...
        // Sort files
        std::sort(sectionFiles.begin(), sectionFiles.end());

        // Navigation bar at top
        server.sendContent("<div class='nav-bar'>");
        server.sendContent("<span>" + sectionTitle + " [Files: " + String(sectionFiles.size()) + "]</span>");
        server.sendContent("</div>");

        // Display files
        server.sendContent("<div class='file-list'>");
        if (sectionFiles.empty()) {
            server.sendContent("<div class='file-item'>[No files]</div>");
        } else {
            for (const String& fileName : sectionFiles) {
                String html = "<div class='file-item'><a href='";
                if (isLocal) {
                    html += "/download?file=" + dirPath.substring(1) + "/" + fileName;
                } else {
                    html += "/remote-download?file=" + dirPath.substring(1) + "/" + fileName + "&node=" + nodeSSID;
                }
                html += "'>&gt; " + fileName + " &lt;</a></div>";
        ...
        server.sendContent("<div class='nav-bar'>");
        server.sendContent("<a href='/node-files?node=" + nodeSSID + "' class='nav-button' onclick='showLoader()'>&lt;&lt; Sections</a>");
        //server.sendContent("<a href='/list' class='nav-button' onclick='showLoader()'>Node List &gt;&gt;</a>");
        server.sendContent("</div>");
    }
...
void handleFileUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        String fullPath = dirPath + "/" + filename;
        Serial.println("Creating file: " + fullPath);
        ...
        if (uploadFile) {
            uploadFile.close();
            Serial.println("Upload complete, file size: " + String(upload.totalSize));
        }
    }
}
