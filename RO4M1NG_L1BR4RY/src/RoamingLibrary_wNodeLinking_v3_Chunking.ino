...
void cleanupStaleTransfers(); //needed by loop
...
void broadcastFiles() {
   ...
   Serial.printf("Number of remote files: %d\n", remoteFiles.size());
    ...
    while (it != failedTransmissions.end()) {
     ...
     // Then broadcast current files from Alexandria
    File root = SD.open("/Alexandria");
    if (!root) {
        Serial.println("Failed to open Alexandria directory");
        return;
    }
...
// Updated cleanupForum function with improved robustness
void cleanupForum() {
...
void handleFileUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        // Create/Open file in Alexandria directory
        uploadFile = SD.open("/Alexandria/" + filename, FILE_WRITE);
...
<style>
body { font-family: 'Courier New', monospace; background-color: #000; color: #0f0; margin: 20px; line-height: 1.6; }
...
.container { border: 1px solid #0f0; padding: 20px; margin: 10px 0; box-shadow: 0 0 10px #0f0; }
.node-item { border-left: 3px solid #0f0; padding: 10px; margin: 10px 0; }
.node-item:hover { background-color: #001100; }
</style>
</head><body>
<div class='container'>
<h3>//404 D3W3Y N07 F0UND//</h3>
    ...
    // Local node
    String localNode = "<div class='node-item'><a href='/node-files?node=" + 
                      String(AP_SSID) + "'>&gt; NODE: " + 
                      String(AP_SSID) + " [LOCAL] &lt;</a></div>";
    server.sendContent(localNode);

    // Remote nodes
    int nodeCount = 0;
    for (const auto& peer : peers) {
        String nodeHtml = "<div class='node-item'><a href='/node-files?node=" + 
                         peer.ssid + "'>&gt; NODE: " + 
                         peer.ssid + " &lt;</a></div>";
        server.sendContent(nodeHtml);
        
        if (++nodeCount % 5 == 0) {  // Every 5 nodes
...
a:hover { color: #fff; text-shadow: 0 0 10px #0f0; }
.container { border: 1px solid #0f0; padding: 20px; margin: 10px 0; box-shadow: 0 0 10px #0f0; }
.file-list { list-style: none; padding: 0; margin: 0; max-height: 0; overflow: hidden; transition: max-height 0.3s ease-out; }
.file-item { border-left: 3px solid #0f0; padding: 10px; margin: 10px 0; }
.file-item:hover { background-color: #001100; }
.file-size { color: #0a0; font-size: 0.9em; }
.section-header { border: 1px solid #0f0; margin: 10px 0; padding: 10px; cursor: pointer; user-select: none; display: flex; justify-content: space-between; }
.section-header:hover { background-color: #001100; }
.section-header::before { content: '[+]'; margin-right: 10px; }
.section-header.active::before { content: '[-]'; }
.section-header.active + .file-list { max-height: 1000px; }
.file-count { color: #0a0; }
</style>
<script>
...
<div class='container'>
    ...
    String headerHtml = "<h3>//NODE: " + nodeSSID + "//</h3>";
    ...
    if (isLocal) {
        // Process local files from Alexandria
        File root = SD.open("/Alexandria");
        if (root) {
            while (File file = root.openNextFile()) {
                String fileName = String(file.name());
                ...
                if (++fileCount % 10 == 0) {  // Every 10 files
                    yield();  // Give system time to process WiFi
                }
            }
            root.close();
    ...
    if (files.empty()) {
        server.sendContent("<p style='color: #0a0;'>[NO FILES]</p>");
    } else {
        int sectionCount = 0;
        for (const auto& group : files) {
            String sectionHtml = "<div class='section-header' onclick='toggleSection(this)'>";
            sectionHtml += "<span>[" + String(group.first) + "]</span>";
            sectionHtml += "<span class='file-count'>[Files: " + String(group.second.size()) + "]</span>";
            sectionHtml += "</div><ul class='file-list'>";
            server.sendContent(sectionHtml);
            
            int fileInSectionCount = 0;
            for (const auto& file : group.second) {
                String fileHtml = "<li class='file-item'>";
                if (isLocal) {
                    fileHtml += "<a href='/download?file=" + file.first + "'>&gt; " + 
                               file.first + " &lt;</a>";
                } else {
                    fileHtml += "<a href='/remote-download?file=" + file.first + 
                               "&node=" + nodeSSID + "'>&gt; " + file.first + " &lt;</a>";
                }
                fileHtml += "<div class='file-size'>[Size: " + 
                           String(file.second / 1024.0, 1) + " KB]</div></li>";
                server.sendContent(fileHtml);
                
                if (++fileInSectionCount % 10 == 0) {
                    yield();
                }
            }
    ...
    static const char HTML_FOOTER[] PROGMEM = R"(
<div style='text-align: center;'>
<br><a href='/list'>&lt;&lt; Return to Node List &gt;&gt;</a>
</div>
</div></body></html>
)";
...
void setup() {
    ...
    if (!SD.exists("/Alexandria")) {
        ...
        Serial.println("Alexandria directory created successfully");
    }
    
    // Clean up and reinitialize forum
    Serial.println("\nStarting forum cleanup...");
    cleanupForum();
    
    // Verify forum structure
    if (!SD.exists("/forum") || !SD.exists("/forum/posts")) {
        Serial.println("Forum directory structure verification failed!");
        Serial.println("Attempting to recreate forum structure...");
        cleanupForum();  // Try one more time
    }
